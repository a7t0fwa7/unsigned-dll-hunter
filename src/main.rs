...
unsafe fn process_file(path: &str) {
    ...
    match status as u32{
        0 => (),
        TRUST_E_NO_SIGNER_CERT => println!("Invalid cert DLL found: {} - result TRUST_E_NO_SIGNER_CERT", &path),
        TRUST_E_COUNTER_SIGNER => println!("Invalid cert found: {} - result TRUST_E_COUNTER_SIGNER", &path),
        TRUST_E_CERT_SIGNATURE => println!("Invalid cert found: {} - result TRUST_E_CERT_SIGNATURE", &path),
        TRUST_E_TIME_STAMP => println!("Expired cert found: {} - result TRUST_E_TIME_STAMP", &path),
        TRUST_E_BAD_DIGEST => println!("Invalid cert found: {} - result TRUST_E_BAD_DIGEST", &path),
        TRUST_E_BASIC_CONSTRAINTS => println!("Invalid cert found: {} - result TRUST_E_BASIC_CONSTRAINTS", &path),
        TRUST_E_FINANCIAL_CRITERIA => println!("Invalid cert found: {} - result TRUST_E_FINANCIAL_CRITERIA", &path),
        TRUST_E_PROVIDER_UNKNOWN => println!("Invalid cert found: {} - result TRUST_E_PROVIDER_UNKNOWN", &path),
        TRUST_E_ACTION_UNKNOWN => println!("Invalid cert found: {} - result TRUST_E_ACTION_UNKNOWN", &path),
        TRUST_E_SUBJECT_FORM_UNKNOWN => println!("Invalid cert found: {} - result TRUST_E_SUBJECT_FORM_UNKNOWN", &path),
        TRUST_E_SUBJECT_NOT_TRUSTED => println!("Invalid cert found: {} - result TRUST_E_SUBJECT_NOT_TRUSTED", &path),
        TRUST_E_NOSIGNATURE => println!("Unsigned DLL found: {} - result TRUST_E_NOSIGNATURE", &path),
        CERT_E_UNTRUSTEDROOT => println!("Invalid cert found: {} - result CERT_E_UNTRUSTEDROOT", &path),
        TRUST_E_FAIL => println!("Invalid cert found: {} - result TRUST_E_FAIL", &path),
        TRUST_E_EXPLICIT_DISTRUST => println!("Explicitly untrusted cert found: {} - result TRUST_E_EXPLICIT_DISTRUST", &path),
        CERT_E_CHAINING => println!("Invalid cert found: {} - result CERT_E_CHAINING", &path),
        TRUST_E_SYSTEM_ERROR => println!("Error: {} - result TRUST_E_SYSTEM_ERROR", &path),
